...
export class ChatService {
  ...
  registerUser(user :ChatUser){
    console.log("Sending a request to")
    console.log(environment.apiUrl)
    return this.httpClient.post("http://localhost:5191/api/chat/register-user", user, {responseType:'text'});
  }

  createChatConnection() {
    this.chatConnection = new HubConnectionBuilder()
      .withUrl('http://localhost:5191/hubs/chat').withAutomaticReconnect().build();
